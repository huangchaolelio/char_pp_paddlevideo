...
### 阶段 8: 用户故事 5 — 上游官方乒乓球样例推理 (US5, P1, 2026-05-12 新增)

**目标**: 在 AI Studio 数据未就绪前, 用上游 BCEBOS 公开提供的乒乓球训练好的权重 + 单样例 pkl 演示完整推理路径, 满足 SC-007.

- [x] T077 [P] [US5] 在 `src/pingpong_av/models/videoswin_tennis.py` 实现 `load_videoswin_tennis_model(checkpoint, repo_root) -> (model, upstream_cfg_dict)`: 读上游 `applications/TableTennis/ActionRecognition/configs/videoswin_tabletennis.yaml` 作为基线; 调用上游 `build_model`; 加载 380MB 权重; 关闭 K400 backbone pretrained (FR-020). **交付**: `src/pingpong_av/models/videoswin_tennis.py` (~90 行).
- [x] T078 [US5] 在 `src/pingpong_av/cli/infer_pkl.py` 实现 `infer-pkl` 子命令: pkl 反序列化 → JPEG 解码 → 均匀采样 (默认 num_seg=32) → ImageNet 标准化 → 喂上游 `RecognizerTransformer` → 输出 `pkl-prediction-v1` schema JSON. checkpoint 缺失时 stderr 给出 curl 下载命令 (FR-021). **交付**: `src/pingpong_av/cli/infer_pkl.py` (~225 行).
- [x] T079 [US5] 在 `src/pingpong_av/cli/__init__.py` 注册 `infer-pkl` 子命令 (替换为第 7 个 CLI). **交付**: `cli/__init__.py` 增加 22 行.
- [x] T080 [US5] 实测 SC-007: 用 `example_tennis.pkl` + `VideoSwin_tennis.pdparams` 跑 `pp infer-pkl --topk 5`, 验证 Top-1 == GT.动作类型 且置信度 ≥ 0.90. **交付**: 实测 Top-1 = 动作7 (id=7), 置信度 0.9999 (远超 SC-007 的 0.90 门槛); ✅ SC-007 通过.

### 阶段 9: 用户故事 6 — 私有 COS 数据集 + BMN 时序定位 (US6, P1, 2026-05-12 新增)

**目标**: 接入团队 COS bucket 上的 AI Studio 竞赛 #127 数据 (43.5GB PP-TSN 特征 + 14 类时序标注), 跑通 BMN 端到端训练.

- [x] T101 [P] [US6] 在 `data/public_datasets.py` 增加 `_ensure_cos_setup` (FR-023) + 流式 `r|gz` 解压 (FR-024) + `_try_read_bmn_features` 视频级单元发现 (FR-025). 凭据从 `.env` 读, 不入 yaml. **交付**: 三个新函数共 ~180 行; 实测 43.5GB tar.gz 5 分钟内解压完毕, 729 .pkl 全部识别.
- [x] T102 [US6] 写 `configs/datasets/pingpong_competition_bmn.yaml` (14 类完整命名 + cos source 段) + `configs/models/bmn_pingpong.yaml` (model.name=bmn + 与 dataset 14 类同步). **交付**: 两份 yaml 通过 `load_config` 校验.
- [x] T103 [US6] 实现 `src/pingpong_av/models/bmn.py::load_bmn_config` + 在 `models/registry.py` 注册. 业务 yaml 仅覆盖 BMN 输入路径与 epochs (FR-027). **交付**: `bmn.py` ~150 行.
- [x] T104 [US6] 写 `scripts/prepare_bmn_inputs.py`: 把上游 GT 转换为 BMN 期望的 `feature/*.npy` (8s 滑窗) + `label_fixed.json` + `label_gts.json`, 自动过滤无 .npy 的条目 (FR-026). 复刻上游 4 个核心函数, **不**复制源码 (章程 VI). **交付**: 脚本 ~250 行; 实测产出 18074 .npy slices.
- [x] T105 [US6] 处理上游 release/2.2.0 在 Python 3.11 / paddle 2.6+ 下 BMN 训练触发的 2 个新兼容性问题: 写 `patches/03-inspect-getargspec-py311.patch` (`inspect.getargspec` 已删除) + `patches/04-record-tensor-scalar-py311.patch` (`Tensor.numpy()[0]` 对 0-d 张量报错). 全部由 `apply_upstream_patches.sh` 幂等应用. **交付**: 2 个 patches, 共 ~30 行 diff.
- [x] T106 [US6] 实测 SC-008: `pp data-prepare → scripts/prepare_bmn_inputs.py → pp train --config configs/models/bmn_pingpong.yaml --allow-dirty`. **交付**:
    - data-prepare: 729 videos, 14 classes, train=621/val=72/test=36 (无泄漏)
    - prepare_bmn_inputs: 18074 .npy slices, label JSON 9MB+10MB
    - pp train: 上游 RecognizerTransformer + BMN, GPU 100% util, loss 1.77 → 0.81 (在 1050 step 内), batch_cost 1.07s; manifest.json 含完整章程 II 四元组. ✅ SC-008 (架构验证) 通过.
- [x] T107 [US6] 在 `src/pingpong_av/upstream_adapter/trainer.py` 增加 `run_upstream_bmn_eval(config_path, ckpt, *, result_path, output_path, label_gts_path, subset, reuse_existing)`: 调用上游 `test_model` 让 `BMNMetric.accumulate()` 做 NMS 后处理 → 写 `bmn_results_<subset>.json` → 再调 `cal_metrics` 拿到 AR@1/5/10/100 + AUC. 注意上游 `BMNMetric` 是通过 `@METRIC.register` (无返回值) 装饰的 class, 必须用 `METRIC.get('BMNMetric')` 拿真实类. 预创建上游硬编码的 `data/bmn/BMN_Test_results/` 目录 (FR-031). **交付**: `trainer.py` +180 行.
- [x] T108 [US6] 在 `src/pingpong_av/cli/eval.py` 增加 `_run_bmn_eval()` 分支: 按 `model.name=bmn` 触发 (FR-029), 调用 `run_upstream_bmn_eval`, 写 `bmn-eval-v1` schema 到 `<run>/metrics.json`. 同步 `manifest.metrics_summary`. 默认 `reuse_existing=True` (FR-030). 扩展 `_find_run_dir` 支持 BMN 的 `<run>/BMN_epoch_NNNNN.pdparams` 布局 (FR-032). **交付**: `eval.py` +160 行.
- [x] T109 [US6] 实测 SC-009: 在 epoch 7/20 ckpt 上跑 `pp eval --split val`. **交付**:
    - 成功复用 cached `bmn_results_validation.json`, 跳过 ~8min GPU 前向
    - 输出 `bmn-eval-v1` schema 完整 JSON, 含 AR@1=28.78%, AR@5=59.17%, AR@10=68.27%, AR@100=80.37%, AUC=74.63%
    - 3111 GT instances vs 196700 proposals, n_videos=1967
    - ✅ SC-009 通过 (AR@100 ≥ 60% 门槛, 实测 80.37%)
...
## 与 002 feature 的关系 (2026-05-13 增补)

`002-raw-video-feature-bmn` 是基于本规范 (001) 之上的**独立增量 feature**, 不修改本任务清单中的任何 T 编号.
其工作内容:

- 复用 001 的 `models/bmn.py + cli/eval.py + scripts/prepare_bmn_inputs.py + 4 patches`
- 新增 3 个 CLI 子命令 (`extract-feat / build-feature-pkls / infer-rawvideo`) 与 1 个工具脚本 (`export_pptsm_inference.py`)
- 新增子包 `src/pingpong_av/extractors/` (clip_id / ffmpeg_frames / manifest / pp_tsm_inference)

详见 `specs/002-raw-video-feature-bmn/{spec,plan,tasks}.md`.

合入 002 后, 本规范的所有验收标准 (SC-001..SC-009) 仍然有效, 不被覆盖.
